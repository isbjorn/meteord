...
if [[ $REBUILD_NPM_MODULES ]]; then
  echo "=> isbjorn/meteord:bin-build is NOT TESTED AT ALL (and maybe not necessary???)"
  ...
  echo "     discourage use of the :bin-build image if possible!  Thanks! -abernix and isbjorn"
  if [ -f /opt/meteord/rebuild_npm_modules.sh ]; then
    cd programs/server
    bash /opt/meteord/rebuild_npm_modules.sh
    cd ../../
  else
    echo "=> Use isbjorn/meteord:bin-build for binary bulding."
    exit 1
  fi
fi
